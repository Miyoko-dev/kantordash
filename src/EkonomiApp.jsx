...
export default function App() {
  ...
  const [travelGoals, setTravelGoals] = useState(() => loadLS("travelGoals", []));
  ...
  useEffect(() => { saveLS("travelGoals", travelGoals); }, [travelGoals]);
  ...
  async function restoreUserSession(sessionUser) {
    ...
    if (Object.keys(cloudData).length > 0) {
      ...
      if (cloudData.travelGoals) setTravelGoals(cloudData.travelGoals);
          ...
          <tbody>
            {rows.map((row, i) => (
              <tr key={i} className="row-hover" style={{ fontWeight: i === 0 ? 700 : 400, background: row.plannedCost > 0 ? "#faf5ff" : row.salaryChange ? "#eff6ff" : i === 0 ? "var(--hover)" : "transparent" }}>
                ...
                <td style={{ color: row.purchaseCost > 0 ? "#0369a1" : "var(--text2)", fontWeight: row.purchaseCost > 0 ? 700 : 400 }}>{row.purchaseCost > 0 ? formatSEK(row.purchaseCost) : "–"}</td>
                ...
                <td style={{ fontSize: 12 }}>
                  ...
                  {(row.purchaseItems||[]).map(p => <span key={p.id} style={{ background: "#e0f2fe", color: "#0369a1", borderRadius: 6, padding: "1px 8px", marginRight: 4, fontWeight: 600, fontSize: 11 }}>🛒 {p.name} {formatSEK(p.cost)}</span>)}
